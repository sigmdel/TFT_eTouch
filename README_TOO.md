...
- [6. Configuring TFT_eTouch](#6-configuring-tft_etouch)
- [7. Setup and Using TFT_eTouch](#7-setup-and-using-tft_etouch)
...
If the PlatformIO environment is used, then  `TFT_eTouchUser.h` can be edited as done in the Arduino IDE. However, it is also  possible to use the usual (local) configuration file `platformio.ini` to define the new macros listed above and the other macros in `TFT_eTouchUser.h`. Do not forget to add the following build flag
